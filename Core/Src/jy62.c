...
{
    if (HAL_UARTEx_ReceiveToIdle_DMA(&huart3, jy62_rx_dma_buf, JY62_UART_RX_BUF_SIZE) != HAL_OK)
    {
        jy62_data.restart_fail_count++;
    }
...
{
    /* 供 HAL_UART_ErrorCallback (vision_parser.c) 兜底 USART3 ORE 后重启 */
    jy62_restart_rx();
}
